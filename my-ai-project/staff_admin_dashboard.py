...
# PythonAnywhere သို့မဟုတ် Local Backend API URL ကို Dynamic ဖတ်ယူခြင်း
...
try:
    ...
    st.warning("⚠️ Backend API Server မပွင့်သေးပါ သို့မဟုတ် URL လွဲနေပါသည်။")
...
with tab_train_brain:
    ...
    try:
        fb_count = requests.get(f"{API_URL}/feedback-count", timeout=5).json()["count"]
    ...
    if csv_upload is not None:
        ...
        if "idea_text" in train_df.columns and "human_score" in train_df.columns:
            ...
            if st.button("Import CSV to Training DB"):
                ...
                resp = requests.post(f"{API_URL}/bulk-feedback", json={"rows": rows}, timeout=300)
    ...
    if docs_uploaded:
        for f in docs_uploaded:
            if f.name not in st.session_state["admin_docs"]:
                resp = requests.post(f"{API_URL}/extract-text", files={"file": (f.name, f.getvalue())}, timeout=120)
                if resp.status_code == 200:
    ...
    if st.button("Trigger Retrain & Hot-Reload Model", type="primary"):
        with st.spinner("Training Model..."):
            resp = requests.post(f"{API_URL}/train-idea-brain", timeout=None)
            if resp.status_code == 200:
                m = resp.json().get("metrics", {})
                st.success(f"Model successfully Retrained & Deployed Live! (Version: `{resp.json().get('version_dir', 'N/A')}`)")
...
with tab_predict:
    try:
        ...
        st.warning(f"Unable to load model info: {e}")